...
test("catalog controls work with pointer and keyboard input", async ({
  page,
}) => {
  await page.goto(
    "/iframe.html?id=components-ark-inspired-catalog--inputs&viewMode=story"
  )
  ...
  await expect(
    page.getByRole("radiogroup", { name: "Experience rating" })
  ).toBeVisible()
})

test("tour opens, advances, and restores focus", async ({ page }) => {
  await page.goto(
    "/iframe.html?id=components-ark-inspired-catalog--surfaces&viewMode=story"
  )
  ...
  await expect(
    page.getByRole("dialog", { name: "Vehicle search" })
  ).toBeVisible()
